  ...
  (
    ...
    <AccordionPrimitive.Header className='flex'>
      <AccordionPrimitive.Trigger
        ref={ref}
        className={cn(
          `flex flex-1 items-center justify-between py-3 font-medium transition-all rounded-md ${
            hover === 'underline'
              ? 'hover:underline'
              : 'hover:bg-accent hover:text-accent-foreground [&[data-state=open]]:bg-gray-200'
          }  [&[data-state=open]>svg]:rotate-180 text-xs`,
          className
        )}
        {...props}
      >
